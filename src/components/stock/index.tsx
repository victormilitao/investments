...
export function Stock(props: StockProps) {
  ...
  return (
    <div className='min-w-[200px] flex gap-3'>
      ...
      <div className='flex flex-col'>
        <span className='text-sm font-bold text-right'>{currencyFormatter.format(props.balance)}</span>
        <span className='text-xs font-normal text-right'>{percentFormatter.format(props.percent)}</span>
      </div>
    </div>
  )
}
